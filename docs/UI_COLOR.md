...
- Web 深色模式以 `#242424` 作为普通页面主底色，分组背景使用更暗的 `#1f1f1f`，卡片和
  浮层依次提高明度；禁止将纯黑作为大面积页面、侧栏或卡片背景。纯黑仅保留给遮罩、阴影
  和媒体渐变。
- 会话选中态必须明显区别于普通 Hover：浅色模式使用 24% 主色混合底，深色模式使用 32%
  亮绿混合底；不得通过降低透明度将选中态弱化成接近普通列表背景。
