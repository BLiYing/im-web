...
- **本地落库 🚧（2026-06-17）**：消息已落 IndexedDB（`src/sdk/localStore.ts`，按 owner 隔离）——收到/同步的消息 + 自己发的(ack 后)都持久化；登录 `preloadLocal` 预载、打开会话秒显、刷新不丢；与服务端同步按 conv_seq 去重。**仍内存态**：会话列表每次从服务端拉、同步位点未持久化（故 🚧 非 ✅）。
- **落后 iOS**：无离线消息+重连增量同步（重连仍从内存基线，非本地最大位点续传）。
