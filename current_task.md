...
## 当前焦点
- **✅ Web 文件入口对齐 iOS（2026-08-01，用户浏览器测试通过；按要求未构建）**：“图片或视频”入口继续使用 `image/video` 与多媒体宫格；“文件”入口不再按 MIME 二次改判，图片、视频和普通文档全部保留浏览器原始 `File` 字节/名称并强制以 `content_type=file + file_name` 发送，聊天与详情统一渲染文件卡片。Code Review 已补齐 IndexedDB 正常消息读取时遗漏的 `fileName`，并新增刷新恢复回归用例；相关 Vitest 按用户要求未运行。
